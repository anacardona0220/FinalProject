...
# Cargamos la imagen y la plantilla.
image = cv2.imread(arguments['image'])
template = cv2.imread(arguments['template'])

# Mostramos tanto la imagen como la plantilla.
cv2.imshow('IMAGEN', resize(image, width=700))
cv2.imshow('PLANTILLA', resize(template, width=700))
